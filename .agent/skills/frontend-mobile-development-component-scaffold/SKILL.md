...
description: "Scaffold production-ready React or React Native components with TypeScript props, accessible interactions, platform-aware styling, tests, stories, and barrel exports. Use for component generation or refactors that need repeatable structure."
risk: unknown
source: community
...
- Creating a new reusable React or React Native component from a written spec.
- Refactoring a component into typed props, predictable state, styles, tests, and barrel exports.
- Generating a platform-specific component variant for web, native, or universal use.
- Adding accessibility, test, or Storybook coverage while preserving component API compatibility.
...
- Derive a component contract first: name, platform, props, events, state ownership, accessibility role, and loading/error states.
- Generate TypeScript props before JSX so component usage is constrained by the public API.
- Keep web and native differences explicit; use React Native primitives for native and DOM semantics for web.
- Include at least one meaningful render test and one interaction test for every interactive component scaffold.
- Export through an `index.ts` barrel only after the component, props, and tests compile.

## NEVER DO THIS

- NEVER scaffold untyped `any` props or event handlers when the expected shape can be expressed.
- NEVER ship placeholder TODO JSX as the final component body; replace placeholders with a minimal working implementation.
- NEVER assume hover-only interactions on mobile; provide touch and keyboard alternatives where relevant.
- NEVER hardcode inaccessible labels, colors, or dimensions without exposing a prop or design token.
- NEVER mix platform-specific imports in a universal component without a clear `.native` / `.web` split or adapter.
...
If the project has Storybook or a component explorer, render the default story and one interactive state before completion.

## Why This Exists

Component scaffolds become copied architecture. A weak scaffold spreads untyped props, inaccessible interactions, placeholder behavior, and platform leakage across the codebase. This skill forces a real component contract and validation evidence before reuse.

## Failure Modes

- The generated component keeps placeholder TODO content and is presented as production-ready.
- Public props use `any`, ambiguous callbacks, or implicit state ownership that consumers cannot reason about.
- A universal component imports DOM or native-only APIs without a platform adapter.
- Tests only assert that rendering does not crash and miss user interactions, disabled states, or errors.
- Accessibility roles, labels, focus/touch behavior, and minimum target sizes are hardcoded or omitted.

## Validation

1. **Type contract** - Run `npx tsc --noEmit`; expect zero errors in generated props, exports, and platform files.
2. **Behavior tests** - Run the repository test command scoped to the component and verify render plus interaction cases pass.
3. **Placeholder scan** - Run `rg -n "TODO|FIXME|:\s*any\b" <component-directory>`; review or remove every match.
4. **Rendered proof** - Open the component in Storybook, a test screen, or the target app and verify default, loading, error, disabled, and interactive states.

## Sub-Agent Propagation

When delegating component scaffolding, require the sub-agent to read `.agent/skills/frontend-mobile-development-component-scaffold/SKILL.md`, define the API before JSX, avoid placeholders and `any`, add behavior tests, and report typecheck plus rendered-state evidence.
