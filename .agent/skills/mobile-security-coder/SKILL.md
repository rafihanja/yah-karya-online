...
description: "Implement secure mobile code for React Native, Expo, Flutter, iOS, and Android with input validation, secure storage, WebView hardening, deep link safety, certificate pinning, privacy controls, and OWASP MASVS-style verification."
risk: unknown
source: community
...
- Classify the data first: public, user private, credential, payment, health, location, or regulated personal data.
- Store credentials and refresh tokens in Keychain/Keystore-backed storage such as SecureStore, not AsyncStorage or plaintext files.
- Validate every deep link, WebView URL, push payload, sensor input, and API response before using it.
- Restrict WebViews with HTTPS allowlists, disabled file access, minimal JavaScript, and a Content Security Policy when rendering HTML.
- Run a mobile security validation pass before release: secret scan, dependency scan, WebView review, storage review, and network transport check.

## NEVER DO THIS

- NEVER put API secrets, signing credentials, private keys, or service-role tokens in a mobile bundle or public env variable.
- NEVER log tokens, PII, biometric outcomes, location traces, push payloads, or raw auth errors.
- NEVER enable arbitrary WebView navigation, `file://` access, or native bridge calls from untrusted content.
- NEVER accept deep link parameters directly into navigation, payment, auth, or destructive actions without allowlisting and validation.
- NEVER disable TLS verification, certificate validation, ATS, or Android Network Security restrictions just to bypass a test failure.
...
npm audit
npx expo-doctor
rg -n "(api[_-]?key|password|secret|token|bearer)" --glob "!node_modules" .
```

Also inspect WebView configuration, deep link handlers, and storage calls manually because automated scans miss mobile-specific context.
...
Mobile binaries run on user-controlled devices where bundles, storage, logs, deep links, and WebViews can be inspected or manipulated. Security must be implemented in storage, navigation, transport, native bridges, permissions, and privacy flows rather than assumed from backend controls.

## Failure Modes

- Tokens or service credentials are stored in AsyncStorage, plaintext files, public env variables, or logs.
- A WebView allows arbitrary navigation, file access, JavaScript bridges, or untrusted injected content.
- Deep link parameters trigger auth, payment, navigation, or destructive actions without validation.
- TLS or certificate validation is disabled to hide a development connectivity problem.
- Analytics, crash reports, push payloads, or screenshots expose PII, location, biometric, or auth data.

## Validation

1. **Dependency review** - Run `npm audit` or the platform dependency scanner and triage mobile-relevant vulnerabilities.
2. **Project health** - Run `npx expo-doctor` or platform checks; resolve insecure or incompatible native modules.
3. **Secret scan** - Run `rg -n "(api[_-]?key|password|secret|token|bearer)" --glob "!node_modules" .` and inspect every match.
4. **Manual control review** - Inspect secure storage, WebView, deep link, permission, network security, backup, logging, and privacy implementations against the threat model.

## Sub-Agent Propagation

When delegating mobile security code, require the sub-agent to read `.agent/skills/mobile-security-coder/SKILL.md`, state the protected data and threat model, avoid insecure storage or WebViews, and return secret-scan plus manual-control evidence.
