...
- Designing or reviewing any iOS, Android, React Native, Flutter, or cross-platform mobile screen.
- Adapting a desktop/web interface to phone or tablet layouts.
- Choosing mobile navigation, gestures, touch target sizing, typography, color, or platform behavior.
- Auditing mobile UX for performance, accessibility, offline behavior, or platform convention risk.

## ALWAYS DO THIS

- Complete the Mobile Feasibility & Risk Index before proposing screens, navigation, or gestures.
- Read the required local reference files listed below before making platform-specific UI decisions.
- Use minimum touch targets: 44pt on iOS and 48dp on Android unless the platform pattern proves otherwise.
- Put primary actions in reachable thumb zones and provide visible fallbacks for gesture-only interactions.
- Validate mobile work on at least a low-width phone viewport and the target platform simulator/device when available.

## NEVER DO THIS

- NEVER shrink a desktop layout and call it mobile design; redesign information density, reach, and navigation.
- NEVER hide required actions behind gestures with no button, menu, or accessible alternative.
- NEVER use long `ScrollView` screens for large datasets; use virtualized lists and stable keys.
- NEVER ignore offline, interruption, low battery, slow network, or one-handed use when defining mobile flows.
- NEVER mix iOS and Android platform conventions without naming the intentional divergence.
...
Mobile interaction is constrained by touch accuracy, reach, interruptions, battery, network quality, and platform conventions. Without a mobile-specific gate, desktop layouts get compressed into unusable screens and gesture, accessibility, offline, and performance failures appear late.

## Failure Modes

- A desktop information hierarchy is reduced in size without redesigning navigation or task priority.
- Primary actions sit outside the thumb zone or use targets smaller than platform guidance.
- A gesture-only action has no visible or accessible fallback.
- Large datasets use non-virtualized scrolling and degrade memory, battery, and frame rate.
- The same navigation, icons, typography, and system behavior are forced onto iOS and Android without justification.

## Validation

1. **Feasibility gate** - Complete the MFRI and confirm platform, framework, navigation, offline, device, and audience inputs.
2. **Reference gate** - Read the required local mobile-design references plus the selected iOS/Android platform files.
3. **Interaction proof** - Test primary flows at phone width with 44pt/48dp touch targets, one-handed reach, keyboard/screen-reader labels, and gesture fallbacks.
4. **Resilience proof** - Verify loading, retry, empty, offline, permission-denied, background/foreground, and low-performance-device states.

## Sub-Agent Propagation

When delegating mobile design, require the sub-agent to read `.agent/skills/mobile-design/SKILL.md` and its mandatory references, name the target platforms, complete the MFRI, avoid desktop compression, and return device-specific accessibility plus interaction evidence.
